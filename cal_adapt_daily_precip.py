...
One netCDF file per year; all files for a given GCM and RCP are merged into a single time series
Data is clipped to a user specified bounding box (lat/long).
    ...
    for rcp in rcps:
        ...
        for cell, name in zip(geom, cell_name):
            ...
            for index, file in enumerate(tqdm(files)):
                ...
                # get daily time series for area of interest (each file is one year so this is annual total)
                # convert kg/m^2/s to inches/day
                # assuming density of water 1000 [kg m-3], value in [kg m-2] is identical to the value in [mm]
                # 25.4mm/inch
                fut_vals = cropped_ds.pr.values * 60 * 60 * 24 / 25.4
                # year = cropped_ds.time.values[0].astype('datetime64[Y]').astype('int') + 1970
                timestamp = cropped_ds.time.values

                df = pd.DataFrame(fut_vals.flatten(), index=timestamp.flatten(), columns=['Total (in/day)'])
                df['SV_GridName'] = name
                df['GCM'] = gcm
                df['RCP'] = rcp
...
out_df.to_csv(os.path.join(deskpth, "daily_totals.csv"))
